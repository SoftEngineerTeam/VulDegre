...
public class MyVisitor extends ASTVisitor {
    ...
    private ArrayList<String> typeInformation = new ArrayList<>();
    ...
    public ArrayList<String> getTypeInformation() {
        return typeInformation;
    }

    public ArrayList<String> getParameterDependencies() {
    ...
    public int visit(IASTDeclaration declaration) {
        ...
        int declarationNum = num; // 保存当前声明节点的编号
        num++;
        //children
        ...
        if (declaration instanceof IASTSimpleDeclaration) {
            ...
            for (IASTDeclarator declarator : simpleDecl.getDeclarators()) {
                String typeName = declSpec.getRawSignature();
                typeInformation.add(num + ", " + typeName);
            }
        }
        ...
        if (declaration instanceof IASTFunctionDefinition) {
            ...
            IASTFunctionDeclarator funcDeclarator = functionDef.getDeclarator();
            if (funcDeclarator instanceof IASTStandardFunctionDeclarator) {
                ...
                for (IASTParameterDeclaration parameter : standardFuncDeclarator.getParameters()) {
                    int parameterNum = num; // 保存参数节点的编号
                    num++; // 更新全局节点编号
                    parameterDependencies.add(declarationNum + ", " + parameterNum);
                }
            }
        }
    ...
    public int visit(IASTExpression expression) {
        ...
        int expressionNum = num; // 保存当前表达式的编号
        num++;// 递增全局编号
        //children
        ...
        if (expression instanceof IASTFunctionCallExpression) {
            ...
            for (IASTInitializerClause argument : arguments) {
                int argumentNum = num; // 保存参数的编号
                num++; // 递增全局编号
                parameterDependencies.add(expressionNum + ", " + argumentNum);
            }
        }
...
//                                    elseVariables.get(elseVariable));
    ...
    public int visit(IASTTypeId typeId) {
        ...
        // TypeInformation
        String typeName = typeId.getRawSignature();
        typeInformation.add(num + ", " + typeName);
