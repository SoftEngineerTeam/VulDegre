...
import VulDecgre.data.data.data_preprocess as DataSplit
...
def get_train_cli_arg_parser():
    ...
    #创建一个参数解析器对象 parser，并设置其描述为 "Train a GNN model."。这个解析器将用于解析命令行参数
    ...
    parser.add_argument(#添加了一个命令行参数，用于指定要训练的 GNN 模型类型，并限制用户只能从预定义的模型类型中进行选择。
        model_param_name,
        type=str,
